...
export default function Product(category: Category) {
  return (
    <div>
      <div className='rouded-xs shadow w-full flex  flex-col gap-3  pt-0 group hover:shadow-xl hover:translate-y-[-5px] duration-300'>
        <Link to={category.title} className='w-full h-40 sm:h-55 group relative'>
          <img className='w-full h-full object-center object-cover' src={category.img} alt={category.title} />
